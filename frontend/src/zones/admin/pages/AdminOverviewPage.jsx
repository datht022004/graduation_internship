    ...
    useEffect(() => {
        async function fetchStats() {
            ...
            try {
                ...
                setRecentCategories(categories.slice(0, 4))
            } catch (error) {
                    ...
                    <>
                        ...
                        <div className="grid gap-6 lg:grid-cols-[1.4fr_0.8fr]">
                            <div className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm sm:p-6">
                                <h2 className="mb-4 text-base font-bold text-slate-800">Bài viết gần đây</h2>
                            ...
                            <div className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm sm:p-6">
                                <h2 className="mb-4 text-base font-bold text-slate-800">Danh mục</h2>
                                    ...
                                    <div className="space-y-2">
                                        {recentCategories.map((category) => (
                                            <div key={category.id} className="flex items-center justify-between rounded-xl bg-slate-50 px-3 py-2">
                                                ...
                                                <span className="rounded-full bg-white px-2 py-0.5 text-xs font-bold text-slate-500">{category.postCount}</span>
                                            </div>
                                        ))}
                                    </div>
                                )}
